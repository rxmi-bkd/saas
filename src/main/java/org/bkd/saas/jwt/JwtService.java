...
public class JwtService {
  ...
  public String createJwt(UUID subject, Role role) {
    ...
    return Jwts.builder().subject(subjectString).issuedAt(issuedAt).expiration(expireAt).claim(ROLE_CLAIM, role).signWith(key).compact();
  }
  ...
  public Claims readJwt(String jwt) {
    try {
      ...
      throw new org.bkd.saas.jwt.JwtException(e.getMessage());
    }
  }
  ...
  public boolean isValidJwt(String jwt) {
    try {
      readJwt(jwt);
      return true;
    } catch (org.bkd.saas.jwt.JwtException e) {
      return false;
    }
  }
}
